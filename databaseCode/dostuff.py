...
# inFileName = input_folder + fileName
if not os.path.exists(output_folder + fileName[:-4] + '/'):
    os.makedirs(output_folder + fileName[:-4] + '/')
parameterFile = open(output_folder + fileName[:-4] + '/parameters.dat', "w+")
parameterFile.write(parametersContent)
rotatexyz.rotate(inFileName, output_folder + fileName[:-4] + '/')
